# Hamsterdam 🐹

**By Olivia Haddon (with parental supervision obvi)**

---

## 👩‍🎨 Creator
**Olivia Haddon** - Author & Creator  

---

## 🐹 Main Characters
- Anne Lotus  
- Frank Holland  
- Detective Crumbsworth 🕵️‍♂️  

---

## 🌆 Setting
Hamsterdam — a hamster city inspired by canals, bridges, tulips, and cozy cafés.

---

## 🧩 Story
When their owner mysteriously disappears while making lunch for a friend, Anne and Frank begin a daring investigation.

With help from the brilliant **Detective Crumbsworth**, they chase a mysterious criminal who keeps getting away. One day, he drops a paper with strange codes on it. As they decode the message, they uncover his identity and the location of his hidden base.

Now they must travel through canals, across bridges, and deep under the city to rescue their owner.

---

## 🍪 Fun Details
- Hamsters can swim through canals  
- Tulip fields throughout the city  
- Stroopwafels are a favorite snack  
- Hidden clues and secret codes  

---

## 🚀 Project Goal
To create a fun, interactive digital storybook using modern web tools like Chakra UI.

---

## 💻 Built With
- HTML, CSS, JavaScript  
- Chakra UI  
- VS Code  

---
